...
NEXT_AGENT_NAME = "brief_writer"
OWN_HANDLE = AGENT_HANDLES[AGENT_NAME]

SYSTEM_PROMPT = "You are the Synthesizer agent in a copyright dispute workflow. Read all findings in the message and produce: a 3-sentence plain-English case summary, the recommended settlement tier (1, 2, or 3), and a confidence level (High, Medium, or Low). Be concise and direct."
...
def load_agent_api_key() -> str:
    if os.getenv("SYNTHESIZER_API_KEY"):
        return os.environ["SYNTHESIZER_API_KEY"]
    config_path = Path("agent_config.yaml")
    with config_path.open(encoding="utf-8") as f:
        config = yaml.safe_load(f) or {}
    api_key = config.get("synthesizer", {}).get("api_key")
    if not api_key:
        raise RuntimeError("Missing synthesizer api_key in agent_config.yaml")
    return api_key
...
def process_message(*, client, api_key, aiml_api_key, chat_id, message):
    ...
    if not message_id or not content:
        return
    say(f"Synthesizer found message: {message_id} {content[:200]}")
    if not should_process_agent(content, AGENT_NAME):
        say(f"Synthesizer skipping message that is not ready for {OWN_HANDLE}: {message_id}")
    ...
    say(f"Synthesizer replying: {llm_response[:300]}")

...
def load_agent_api_key() -> str:
    if os.getenv("BRIEFWRITER_API_KEY"):
        return os.environ["BRIEFWRITER_API_KEY"]
    config_path = Path("agent_config.yaml")
    with config_path.open(encoding="utf-8") as f:
        config = yaml.safe_load(f) or {}
    api_key = config.get("brief_writer", {}).get("api_key")
    if not api_key:
        raise RuntimeError("Missing brief_writer api_key in agent_config.yaml")
    return api_key
...
def poll_forever() -> None:
    ...
    say("BriefWriter REST poller starting")
